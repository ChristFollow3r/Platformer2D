...
[CustomEditor(typeof(CookingRecipe))]
public class CookingRecipeEditor : Editor
{
    // ── Layout constants ──────────────────────────────────────────────────
    ...
    private static readonly Color ColTimeBg = new(0.20f, 0.15f, 0.10f, 1f);
    private static readonly Color ColTimeBorder = new(0.80f, 0.55f, 0.20f, 1f);

    // ── Serialised properties ─────────────────────────────────────────────
    private SerializedProperty _result;
    private SerializedProperty _amount;
    private SerializedProperty _cookTime;
    ...
    private void OnEnable()
    {
        ...
        _cookTime = serializedObject.FindProperty("cookTime");
    ...
    // ── Cook Time ─────────────────────────────────────────────────────────
    private void DrawCookTimeSection()
    {
        GUILayout.Label("Cook Time", EditorStyles.boldLabel);

        EditorGUILayout.BeginHorizontal(EditorStyles.helpBox);

        // Flame icon area
        Rect iconRect = GUILayoutUtility.GetRect(40f, 40f, GUILayout.Width(40f), GUILayout.Height(40f));
        DrawSlotBackground(iconRect, true, ColTimeBg, ColTimeBorder, ColTimeBg, ColTimeBorder);
        GUI.Label(iconRect, "🔥", new GUIStyle(EditorStyles.label)
        {
        ...
        EditorGUILayout.BeginVertical();
        GUILayout.Space(6);
        EditorGUILayout.LabelField("Duration (seconds)", EditorStyles.miniLabel);
        _cookTime.floatValue = Mathf.Max(0f, EditorGUILayout.FloatField(_cookTime.floatValue));

        // Friendly display
        float t = _cookTime.floatValue;
        string friendly = t >= 60f
            ? $"{Mathf.FloorToInt(t / 60f)}m {Mathf.FloorToInt(t % 60f)}s"
            : $"{t:0.##}s";
        EditorGUILayout.LabelField(friendly, EditorStyles.centeredGreyMiniLabel);
        EditorGUILayout.EndVertical();
    ...
    {
        GUILayout.Label("Ingredients  (2 × 2)", EditorStyles.boldLabel);
        GUILayout.Space(4f);

        float gridWidth = 2 * SlotSize + SlotPad;
        ...
        for (int row = 0; row < 2; row++)
        {
            ...
            for (int col = 0; col < 2; col++)
            {
                ...
                DrawSlotBackground(rect, item != null,
                    ColSlotEmpty, ColBorderEmpty,
                    ColSlotFilled, ColBorderFilled);
                DrawSlotContent(rect, item);
                HandleSlotInteraction(rect, slot, index);

                if (col < 1) GUILayout.Space(SlotPad);
            }
            EditorGUILayout.EndHorizontal();
            if (row < 1) GUILayout.Space(SlotPad);
        }
